...
const fetchEntries = async () => {
    loading.value = true;
    try {
        const response = await api.get(`/${currentType.value}`);
...
const deleteEntry = async (id) => {
    if (!confirm(`Are you sure you want to delete this ${currentType.value.slice(0, -1)}?`)) return;
    alert(`Delete ${currentType.value.slice(0, -1)}...`);
    try {
        await api.delete(`/${currentType.value}/${id}`);
