<template>
  <div class="classroom" :class="{ 'minimize-video': minimizeVideo }">
    <!-- LEFT SIDE -->
    <div class="left-pane" :class="{ 'minimize-video': minimizeVideo }" :style="{ flex: minimizeVideo ? '1' : '2' }">
      <div class="header-section">
        <h2>Live Classroom</h2>
        <button class="minimize-btn" @click="toggleMinimizeVideo" :title="minimizeVideo ? 'Maximize' : 'Minimize'">
          {{ minimizeVideo ? '🔼' : '➖' }}
        </button>
        <!-- explicit maximize button when minimized for clarity -->
        <button v-if="minimizeVideo" class="maximize-btn" @click="toggleMinimizeVideo" title="Restore layout">⤢
          Maximize</button>
      </div>

      <!-- VIDEO GRID -->
      <div class="videos" :class="{ minimized: minimizeVideo }">
        <div class="video-tile" :class="{ minimized: localState === 'minimized', closed: localState === 'closed' }">
          <div class="video-label">
            You
            <div class="video-controls">
              <button @click.stop="toggleLocalMinimize" :title="localState === 'minimized' ? 'Restore' : 'Minimize'">{{
                localState === 'minimized' ? '🔼' : '➖' }}</button>
              <button @click.stop="restoreLocal" title="Restore">🔁</button>
              <button @click.stop="closeLocal" title="Close">✕</button>
            </div>
          </div>
          <video v-if="localState !== 'closed'" ref="localVideo" autoplay playsinline muted
            :class="{ small: localState === 'minimized' }"></video>
          <div v-else class="video-closed">Local video closed</div>
        </div>

        <div class="video-tile" :class="{ minimized: remoteState === 'minimized', closed: remoteState === 'closed' }">
          <div class="video-label">
            Tutor
            <div class="video-controls">
              <button @click.stop="toggleRemoteMinimize"
                :title="remoteState === 'minimized' ? 'Restore' : 'Minimize'">{{
                  remoteState === 'minimized' ? '🔼' : '➖' }}</button>
              <button @click.stop="restoreRemote" title="Restore">🔁</button>
              <button @click.stop="closeRemote" title="Close">✕</button>
            </div>
          </div>
          <video v-if="remoteState !== 'closed'" ref="remoteVideo" autoplay playsinline
            :class="{ small: remoteState === 'minimized' }"></video>
          <div v-else class="video-closed">Remote video closed</div>
        </div>
      </div>
    ...
    <div class="right-pane" :class="{ minimized: minimizeVideo }">
      <div class="chat-header">
        ...
        <button class="close-btn" @click="toggleMinimizeVideo" v-if="minimizeVideo" title="Close">✕</button>
      </div>
...
// Per-video states
const localState = ref('normal'); // 'normal' | 'minimized' | 'closed'
const remoteState = ref('normal');
...
// Video controls for local
function toggleLocalMinimize() {
  localState.value = localState.value === 'minimized' ? 'normal' : 'minimized';
}
function restoreLocal() {
  localState.value = 'normal';
  if (localStream && localVideo.value) localVideo.value.srcObject = localStream;
}
function closeLocal() {
  localState.value = 'closed';
  if (localStream) {
...
// Video controls for remote
function toggleRemoteMinimize() {
  remoteState.value = remoteState.value === 'minimized' ? 'normal' : 'minimized';
}
function restoreRemote() {
  remoteState.value = 'normal';
}
function closeRemote() {
  remoteState.value = 'closed';
  if (remoteVideo.value && remoteVideo.value.srcObject) {
    const s = remoteVideo.value.srcObject;
...
function toggleMinimizeVideo() {
  ...
  console.log('toggleMinimizeVideo ->', minimizeVideo.value);
}
...
function toggleRecording() {
  if (!isRecording.value) {
    recordedChunks = [];
    recorder = new MediaRecorder(localStream || screenStream);
...
function toggleVolume() {
  ...
  if (remoteVideo.value) remoteVideo.value.muted = !volumeOn.value;
}
...
function createPeerConnection() {
  ...
  pc.ontrack = (e) => {
    // attach remote stream and restore remoteState
    try {
      remoteVideo.value.srcObject = e.streams[0];
      remoteState.value = 'normal';
    } catch (err) {
      console.warn('Error attaching remote stream', err);
    }
  };
...
function stopScreenShare() {
  ...
  if (localStream && pc) {
    const senders = pc.getSenders();
    const videoSender = senders.find((s) => s.track && s.track.kind === 'video');
    if (videoSender && localStream.getVideoTracks()[0]) videoSender.replaceTrack(localStream.getVideoTracks()[0]);
  }
}
...
/* layout and styles omitted for brevity in patch; keep the project's CSS here */
/* keep the rest of the CSS inside the one <style scoped> block below */
