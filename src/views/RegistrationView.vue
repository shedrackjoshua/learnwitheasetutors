...
const nextStep = () => {
    // if parent say "yes", move to child step
...
const addChildAndContinue = () => {
    // push a shallow copy of the current child and reset
    form.value.children.push({ ...child.value });
    // reset child fields for next entry
    child.value = {
        wards_name: '',
        gender: '',
        students_email: '',
    ...
    // if parent wants to register another child, go to next child
    if (form.value.register_another_child === true) {
        step.value++;
    } else {
        // if parent says "no",
...
const submitForm = async () => {
    ...
    // If the current child has data but wasn't added via "Next", add it now
    if (!isChildEmpty()) {
        form.value.children.push({ ...child.value });
        // reset child
        child.value = {
    ...
    try {
        const response = await axios.post('http://localhost:3000/registrations', form.value);
        ...
        // clear Forms fields in 3 seconds
        setTimeout(() => {
            submitMessage.value = '';
        }, 3000);
    } catch (error) {
        console.error('Registration error:', error);
        submitError.value = error.response?.data?.message || 'There was an error with your registration, please try again later';
    } finally {
